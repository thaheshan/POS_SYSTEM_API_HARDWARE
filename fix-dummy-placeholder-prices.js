...
// Sri Lanka Hardware market price map for PVC fittings & baseline catalog items
const priceMap = [
...
async function fixDummyPlaceholderPrices() {
  ...
  console.log(`Auditing products with Rs. 0 or Rs. 100 dummy placeholder prices for Shop ${shop.id}...`);
  ...
  const dummyPriceProducts = products.filter(p => {
    const s = Number(p.sellingPrice || 0);
    const c = Number(p.purchasePrice || 0);
    return s === 0 || s === 100 || c === 0 || c === 100;
  });

  console.log(`\nFound ${dummyPriceProducts.length} products with Rs. 0 or Rs. 100 dummy prices:`);

  if (dummyPriceProducts.length === 0) {
    console.log('🎉 No products with Rs. 0 or Rs. 100 dummy prices found! All prices are clean.');
    return;
  }

  let fixedCount = 0;

  for (const prod of dummyPriceProducts) {
    const nameLower = prod.name.toLowerCase();
    const matchedFix = priceMap.find(fix => fix.keywords.some(kw => nameLower.includes(kw)));

    let newSelling = matchedFix ? matchedFix.selling : 150;
    let newCost = matchedFix ? matchedFix.cost : 100;
    ...
    fixedCount++;
    console.log(`✅ Fixed ${prod.sku} - "${prod.name}": Changed (Selling: Rs.${prod.sellingPrice}, Cost: Rs.${prod.purchasePrice}) -> Selling: Rs. ${newSelling}, Cost: Rs. ${newCost}`);
  }

  console.log(`\n🎉 SUCCESSFULLY CORRECTED ALL ${fixedCount} DUMMY Rs. 0 AND Rs. 100 PRICE PRODUCTS!`);
}

fixDummyPlaceholderPrices()
  .catch(console.error)
  .finally(() => {
    prisma.$disconnect();
    if (pool) pool.end();
