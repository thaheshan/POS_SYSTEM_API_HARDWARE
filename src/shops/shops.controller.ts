...
export class ShopsController {
  ...
  async uploadLogo(@Req() req: AuthRequest, @UploadedFile() file: any) {
    if (!file) {
  ...
  async verifyShopAssociation(@Body() verifyShopDto: VerifyShopDto) {
    const result = await this.shopsService.verifyShopAssociation(
      verifyShopDto.shopId,
      verifyShopDto.privateId,
    );
